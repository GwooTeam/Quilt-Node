...
function setClientSocket(clientId, clientSocket) {
  clientSocket.on('data', data => {
        ...
        if (message === 'auth request') { 
           ...
           // Send the random value to the client
           clientSocket.write(`Random Value: ${randomValue}`);
...
function nonce_verify_raw(dataVal, signVal, pukVal) {
  //console.log('command: ' + `../../DigitalSignature/dmodule -v -r ${dataVal} ${signVal} ${pukVal}`);
  try {
