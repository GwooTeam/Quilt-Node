...
const dmodule_path = config.dmodule_path;
...
const command =
  // "wsl bash -c \"export LD_LIBRARY_PATH='/mnt/d/Dev/2/DigitalSignature' && /mnt/d/Dev/2/DigitalSignature/dmodule --keygen -r\"";
  `wsl (export LD_LIBRARY_PATH=../../DigitalSignature; ${dmodule_path} --keygen -r)`;
function keygen_sign(callback) {
  ...
  // console.log("keygen_out: " + keygen_out);
...
function nonce_sign_raw(dataVal, prkVal) {
  let sign_out = execSync(
    `wsl (export LD_LIBRARY_PATH=../../DigitalSignature; ${dmodule_path} -s -r ${dataVal} ${prkVal})`,
    (error, stdout, stderr) => {
      if (error) {
        console.error(`Execution error: ${error.message}`);
        return;
