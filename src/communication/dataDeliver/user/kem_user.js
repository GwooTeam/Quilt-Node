...
function kem_decapsulate(prkVal, capVal) {
  let decap_out = execSync(`export LD_LIBRARY_PATH=./KEM/modules && ./KEM/modules/kmodule -r --decap --key=${prkVal} --target=${capVal}`, (error, stdout, stderr) => {
      if (error) {
...
function ssk_encrypt(sskVal, dataVal) {
  let enc_out = execSync(`export LD_LIBRARY_PATH=./KEM/modules && ./KEM/modules/kmodule -r --encrypt --key=${sskVal} --target=${dataVal}`, (error, stdout, stderr) => {
    if (error) {
        console.error(`Execution error: ${error.message}`);
        return;
    }
