...
function kem_encapsulate(pukVal) {
  try {
    ...
    let encap_out = execSync(
      `export LD_LIBRARY_PATH=./KEM/modules && ./KEM/modules/kmodule --encap -r --key=${pukVal}`
    );
...
function ssk_decrypt(sskVal, encVal) {
  let dec_out = execSync(
    `export LD_LIBRARY_PATH=./KEM/modules && ./KEM/modules/kmodule --decrypt -r --key=${sskVal} --target=${encVal}`,
    (error, stdout, stderr) => {
      if (error) {
        console.error(`Execution error: ${error.message}`);
        return;
